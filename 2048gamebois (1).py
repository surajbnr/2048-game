...
class Game(tk.Tk):
    ...
    def create_widgets(self):
        ...
        tk.Button(self.buttonframe, text = "New Game",command=self.newgame).grid(row=0, column=0)
        ...
        tk.Label(self.buttonframe, text = "High Score:").grid(row=0, column=3)
        tk.Label(self.buttonframe, textvariable=self.highscorestring).grid(row=0, column=4)        
        self.buttonframe.pack(side="top")

    #executes moves based on keys pressed
    ...
    #It returns True if board is full & has no more moves
    def boardfull(self):
        for i in range(0,4):
            for j in range(0,4):
...
if __name__ == "__main__":
    root = Game()
    root.bind_all('<Key>', root.keypressed)
    root.wm_title("2048")
    root.minsize(420,450)
    root.maxsize(420,450)
    root.mainloop()
